...
- **Conv2DTranspose cross-runtime weight transfer.** `src/weight_converter.js` does not have an explicit Conv2DTranspose handler — its tensor layout differs from Conv2D in PyTorch vs TF.js. No shipped demo trains a Conv2DTranspose layer in one runtime and loads it in the other, so this gap is theoretical today; it would need ~20 lines to add when a demo first needs it.
