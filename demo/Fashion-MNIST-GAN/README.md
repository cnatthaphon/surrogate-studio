...
> **Note on MLP-WGAN volatility:** WGAN with `clipWeights: 0.1` (matching Arjovsky 2017's loose-clipping baseline) is sensitive to RNG initialization — different training runs land in different basins. The shipped pre-trained file converged to a usable basin but tends to produce noisier samples than the BCE-based MLP-GAN/DCGAN. Retrain via the `MLP-WGAN Trainer` card if you want to try for a cleaner basin.
