  ...
  window.FASHION_MNIST_TRANSFORMER_PRESET = {
    ...
    trainers: [
      ...
      { id: "t-vit-mlp-head", name: "ViT + MLP Head Trainer", schemaId: sid, datasetId: DS, modelId: "m-vit-mlp-head", status: "draft",
        config: { epochs: 30, batchSize: 64, learningRate: 0.001, optimizerType: "adam", useServer: true,
                  earlyStoppingPatience: 10, lrSchedulerType: "plateau", lrPatience: 5, lrFactor: 0.5 } },
      // Pre-trained (PyTorch CUDA, 30 epochs, 10K samples)
      { id: "t-tiny-vit-pre", name: "Tiny ViT (pre-trained)", schemaId: sid, datasetId: DS, modelId: "m-tiny-vit", status: "done",
        _pretrainedVar: "TINY_VIT_PRETRAINED_BIN_B64",
        config: { epochs: 30, batchSize: 64, learningRate: 0.001, optimizerType: "adam" } },
      { id: "t-small-vit-pre", name: "Small ViT (pre-trained)", schemaId: sid, datasetId: DS, modelId: "m-small-vit", status: "done",
        _pretrainedVar: "SMALL_VIT_PRETRAINED_BIN_B64",
        config: { epochs: 30, batchSize: 64, learningRate: 0.001, optimizerType: "adam" } },
      { id: "t-vit-mlp-head-pre", name: "ViT + MLP Head (pre-trained)", schemaId: sid, datasetId: DS, modelId: "m-vit-mlp-head", status: "done",
        _pretrainedVar: "VIT_MLP_HEAD_PRETRAINED_BIN_B64",
        config: { epochs: 30, batchSize: 64, learningRate: 0.001, optimizerType: "adam" } },
    ],
    generations: [],
    evaluations: [
      {
        ...
        name: "ViT Classification Benchmark",
        schemaId: sid,
        datasetId: DS,
        trainerIds: ["t-tiny-vit-pre", "t-small-vit-pre", "t-vit-mlp-head-pre"],
