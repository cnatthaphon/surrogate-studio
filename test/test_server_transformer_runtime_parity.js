/**
 * Cross-runtime transformer parity regression test.
 *
 * Verifies that TF.js and the server PyTorch reload path produce the same
 * outputs for:
 * 1. PatchEmbed -> Flatten
 * 2. PatchEmbed -> TransformerBlock -> GlobalAvgPool1D
 *
 * This guards the ViT demo path and named checkpoint loading for patch/embed
 * and transformer submodules.
 */
...
if (!_pyCheck) {
  console.log("SKIP test_server_transformer_runtime_parity (python not available: " + PYTHON + ")");
  process.exit(0);
}
...
async function runCase(name, graph, featureSize, xInput) {
  ...
  var cfgPath = path.join("/tmp", "surrogate_studio_transformer_parity_" + name + ".json");
  ...
  assert(maxDiff < 1e-5, name + " matches server reload path (maxDiff=" + maxDiff + ")");
...
(async function main() {
  console.log("=== 1. PatchEmbed Parity ===");
  await runCase(
    "patch_embed_flatten",
    G([
      ["image_source", { sourceKey: "pixel_values", featureSize: 784, imageShape: [28, 28, 1] }],
      ["patch_embed", { patchSize: 7, embedDim: 16, useBias: true }],
      ["flatten", {}],
      ["output", { target: "custom", targetType: "custom", loss: "none" }],
    ]),
    784,
    Array.from({ length: 3 }, function (_, r) {
      return Array.from({ length: 784 }, function (_, c) { return ((r + 1) * ((c % 17) + 1)) / 500; });
    })
  );

  console.log("\n=== 2. Transformer Block Parity ===");
  await runCase(
    "patch_embed_transformer_gap",
    G([
      ["image_source", { sourceKey: "pixel_values", featureSize: 784, imageShape: [28, 28, 1] }],
      ["patch_embed", { patchSize: 7, embedDim: 16, useBias: true }],
      ["transformer_block", { numHeads: 4, ffnDim: 32, dropout: 0.0, epsilon: 1e-3 }],
      ["global_avg_pool1d", {}],
      ["output", { target: "custom", targetType: "custom", loss: "none" }],
    ]),
    784,
    Array.from({ length: 3 }, function (_, r) {
      return Array.from({ length: 784 }, function (_, c) { return ((r + 2) * ((c % 19) + 1)) / 700; });
    })
  );
